...
class FrontView: NSObject {
    ...
    convenience override init() {
        ...
        for group in frontResult {
            dict.append(Torso(withIndex: Int(group.id), color: (group.color?.colorFromString())!, muscleName: group.alias!, tapArea: tapPaths[index]))
            index+=1
        }
...
class BackView: NSObject {
    ...
    convenience override init() {
        ...
        for group in backResult {
            dict[index] = Torso(withIndex: Int(group.id), color: (group.color?.colorFromString())!, muscleName: group.alias!, tapArea: tapPaths[index])
            index+=1
        }
